...
from transformers import AutoTokenizer, Rwkv6Config, is_torch_available
...
class Rwkv6ModelTester:
    ...
    def get_large_model_config(self):
        return Rwkv6Config.from_pretrained("RWKV/v6-Finch-1B6-HF")
    ...
    def create_and_check_rwkv_model(self, config, input_ids, input_mask, head_mask, token_type_ids, *args):
        ...
        model = Rwkv6Model(config=config)
...
@require_torch
class Rwkv6ModelTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (Rwkv6Model, Rwkv6ForCausalLM) if is_torch_available() else ()
    pipeline_model_mapping = (
        {"feature-extraction": Rwkv6Model, "text-generation": Rwkv6ForCausalLM} if is_torch_available() else {}
    )
    all_generative_model_classes = (Rwkv6ForCausalLM,) if is_torch_available() else ()
    fx_compatible = False
    ...
    def setUp(self):
        self.model_tester = Rwkv6ModelTester(self)
        self.config_tester = ConfigTester(
            self, config_class=Rwkv6Config, n_embd=37, common_properties=["hidden_size", "num_hidden_layers"]
        )
    ...
    def test_model_from_pretrained(self):
        ...
        model = Rwkv6Model.from_pretrained(model_name)
...
@slow
class RWKV6IntegrationTests(unittest.TestCase):
    def setUp(self):
        self.model_id = "RWKV/v6-Finch-1B6-HF"
        self.tokenizer = AutoTokenizer.from_pretrained(self.model_id)

    def test_simple_generate(self):
        expected_output = "Hello my name is Jasmine and I am a newbie to the"
        model = Rwkv6ForCausalLM.from_pretrained(self.model_id).to(torch_device)
    ...
    def test_simple_generate_bf16(self):
        ...
        input_ids = self.tokenizer("Hello my name is", return_tensors="pt").input_ids.to(torch_device)
        model = Rwkv6ForCausalLM.from_pretrained(self.model_id, torch_dtype=torch.bfloat16).to(torch_device)
